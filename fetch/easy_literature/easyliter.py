...
def set_args():
    ...
    parser.add_argument(
        ...
        "--input",
        type=str,
        default="output/md/liter_input.md",
...
def literature_search(
    input_path="output/md/liter_input.md",
    output_path="output/pdf",
    delete_bool=False,
    proxy=None,
    migration_path="output/pdf_new",
    gproxy_mode="single",
):
    if output_path:
...
def literature_search_api(
    ...
    save_note_path = os.path.join(
        output_path, "md", input_path.split("/")[-1].split(".")[0] + "_literature.md"
    )
    save_json_path = os.path.join(
        output_path, "doc", input_path.split("/")[-1].split(".")[0] + "_literature.json"
    )
        ...
        if bib:
            ...
            if search_idx == "title":
                if convert_title_pdfname(keyword) != pdf_name:
                    logger.info(
                        f"The paper's title: {keyword}; The search pdf name: {pdf_name}."
                    )
                    continue
...
def main():
    input_path, output_path, delete_bool, proxy, migration_path, gproxy_mode = (
        check_args()
    )
    literature_search(
        input_path, output_path, delete_bool, proxy, migration_path, gproxy_mode
