...
def llm_analysis_search_list(search_list, engine: Literal['gpt', 'moonshot']='moonshot', num: int=5):
    ...
    if os.path.exists(json_path):
        ...
        aigc_task_names = [AIGC_TASK_TYPES[i]+"({})".format(AIGC_TASK_TYPES_ENGLISH[i]) for i in range(len(AIGC_TASK_TYPES))]
        search_results = {}
        ...
        for search in tqdm(search_list, total=paper_num):
            ...
            paper_all = sum([len(search_results[aigc_task_name]) for aigc_task_name in aigc_task_names])
            if paper_all >= num:
                break

        # 将论文列表保存到JSON文件中
        json.dump(search_results, open(json_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=4)
...
def export_pdf_md(search_results):
    ...
    for task_name, papers in search_results.items():
        arxiv_ids = []
        save_md_cur_file = os.path.join(save_md_dir, md_file_name_template.format(task_name))
